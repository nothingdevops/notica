...
window_hours    INTEGER NOT NULL DEFAULT 24 -- time window để gom alerts
filter_status   VARCHAR[] DEFAULT NULL      -- NULL = all statuses
filter_tags     JSONB DEFAULT NULL          -- NULL = all tags
...
UNIQUE (schedule_id, window_start, window_end)  -- idempotency
```
...
```
Dùng cho: `retention_days` (default 90), system-level config.
...
1. Job fires theo `cron_expr`
2. Query alerts trong `window_hours` gần nhất, filter theo `filter_status` và `filter_tags`
3. Nếu không có alerts → skip (không gửi empty digest)
4. Format Adaptive Card (summary bar + table rows)
5. Gửi đến từng contact
6. Insert vào `schedule_executions` để track (idempotency + history)

### Teams Notification Format
- **Immediate**: Adaptive Card FactSet (job name, status, time, duration, error snippet)
- **Digest**: Adaptive Card ColumnSet table (header row + data rows, mỗi row 1 alert)
...
frontend/src/
  features/
    ...
    settings/       # Retention config + system settings
  components/ui/    # shadcn primitives
  lib/
    api.ts          # fetch wrapper + query keys
    queryClient.ts
  hooks/
    useDebounce.ts
    usePagination.ts
...
| Settings | `/settings` | Retention period, app config |
...
- **SSO / Keycloak integration** — toàn bộ UI và API đằng sau Keycloak OIDC
